...
function Initialize-WorkContext {
    param (
...
"@
        Write-Log -Level INFO -Message $logMessage -Category "작업컨텍스트" -ProblemId $ProblemId
        # 환경 정보 로깅
        $envInfo = $script:EnvironmentInfo | ConvertTo-Json
        Write-Log -Level INFO -Message "환경 정보:`n$envInfo" -Category "환경" -ProblemId $ProblemId
    }
    catch {
        Write-Log -Level ERROR -Message "작업 컨텍스트 초기화 실패" -Result $_.Exception.Message -Category "작업컨텍스트"
        throw
    }
}
function Add-DirectoryChange {
    param (
        [Parameter(Mandatory=$true)]
        [string]$Action,
        [Parameter(Mandatory=$true)]
    ...
    if (!$script:WorkContext) {
        Write-Log -Level ERROR -Message "작업 컨텍스트가 초기화되지 않았습니다." -Category "작업컨텍스트"
        return
    }
    $change = @{
        Timestamp = Get-Date
...
function Test-DirectoryStructure {
    ...
    try {
        ...
        Write-Log -Level ERROR -Message "디렉토리 구조 검증 실패" -Result $_.Exception.Message
        throw
    }
}
function New-ServiceDirectory {
    ...
    try {
        ...
        if (!(Test-Path $servicePath)) {
            Write-Log -Level INFO -Message "서비스 디렉토리 생성: $ServiceName" -Category "디렉토리"
            New-Item -Path $servicePath -ItemType Directory -Force | Out-Null
            Add-DirectoryChange -Action "Create" -Path $servicePath -Details "서비스 디렉토리 생성"
            # 서비스별 하위 디렉토리 생성
            @('src', 'tests', 'docs', 'config') | ForEach-Object {
                $subDir = Join-Path $servicePath $_
                New-Item -Path $subDir -ItemType Directory -Force | Out-Null
                Add-DirectoryChange -Action "Create" -Path $subDir -Details "서비스 하위 디렉토리 생성"
            }
            Write-Log -Level INFO -Message "서비스 디렉토리 생성 완료" -Result "Success" -Category "디렉토리"
        }
        else {
            Write-Log -Level WARNING -Message "이미 존재하는 서비스 디렉토리: $ServiceName" -Category "디렉토리"
            Add-DirectoryChange -Action "Skip" -Path $servicePath -Details "이미 존재하는 디렉토리" -Result "Skipped"
        }
    }
    catch {
        Write-Log -Level ERROR -Message "서비스 디렉토리 생성 실패: $ServiceName" -Result $_.Exception.Message -Category "디렉토리"
        Add-DirectoryChange -Action "Error" -Path $servicePath -Details $_.Exception.Message -Result "Failed"
        throw
    }
}
function Initialize-DirectoryStructure {
    ...
    try {
        # 기본 디렉토리 구조 검증
        $structureCheck = Test-DirectoryStructure -BasePath $BasePath
        if (!$structureCheck.IsValid -or $Force) {
            Write-Log -Level INFO -Message "디렉토리 구조 초기화 시작"
            # 서비스 디렉토리 생성
            foreach ($service in $script:ServiceDirectories) {
                New-ServiceDirectory -BasePath $BasePath -ServiceName $service
            }
            # 공통 디렉토리 생성
            foreach ($dir in $script:CommonDirectories) {
                ...
                if (!(Test-Path $path)) {
                    ...
                    Write-Log -Level INFO -Message "공통 디렉토리 생성: $dir"
                }
            }
            Write-Log -Level INFO -Message "디렉토리 구조 초기화 완료" -Result "Success"
        }
        else {
            Write-Log -Level INFO -Message "디렉토리 구조가 이미 올바르게 설정되어 있습니다."
        }
    }
    catch {
        Write-Log -Level ERROR -Message "디렉토리 구조 초기화 실패" -Result $_.Exception.Message
        throw
    }
}
# 상태 보고서 생성
...
"@
            ...
            Write-Log -Level INFO -Message "HTML 보고서 생성 완료" -Result $htmlReportPath -Category "보고서"
            # Markdown 요약 보고서 생성
            $null = New-MarkdownReport -BasePath $BasePath -Status $status
            # JSON 상태 보고서 생성
            $null = New-JsonReport -BasePath $BasePath -Status $status
        }
        return $status
    }
    catch {
        Write-Log -Level ERROR -Message "디렉토리 상태 보고서 생성 실패" -Result $_.Exception.Message -Category "보고서"
        throw
    }
}
# 보고서 관련 함수들
...
"@
        ...
        Write-Log -Level INFO -Message "Markdown 요약 보고서 생성 완료" -Result $reportPath -Category "보고서"
        return $reportPath
    }
    catch {
        Write-Log -Level ERROR -Message "Markdown 보고서 생성 실패" -Result $_.Exception.Message -Category "보고서"
        throw
    }
}
function New-JsonReport {
    ...
    try {
        ...
        Write-Log -Level INFO -Message "JSON 상태 보고서 생성 완료" -Result $reportPath -Category "보고서"
        return $reportPath
    }
    catch {
        Write-Log -Level ERROR -Message "JSON 보고서 생성 실패" -Result $_.Exception.Message -Category "보고서"
        throw
    }
}
function Backup-ExistingFile {
    ...
    if (Test-Path $FilePath) {
        try {
            ...
            Write-Log -Level INFO -Message "파일 백업 생성" -Result "원본: $FilePath -> 백업: $backupPath" -Category "백업"
            return $backupPath
        }
        catch {
            Write-Log -Level ERROR -Message "파일 백업 실패: $FilePath" -Result $_.Exception.Message -Category "백업"
            throw
        }
    }
    return $null
}
function New-ProjectManual {
    param (
        [Parameter(Mandatory=$true)]
        [string]$BasePath,
...
"@ | Out-File -FilePath $userGuide -Encoding UTF8
        Write-Log -Level INFO -Message "프로젝트 매뉴얼 생성 완료" -Category "문서"
        return @{
        ...
        Write-Log -Level ERROR -Message "매뉴얼 생성 실패" -Result $_.Exception.Message -Category "문서"
        throw
    }
}
# Export-ModuleMember에 새로운 함수들 추가
Export-ModuleMember -Function Initialize-WorkContext, Initialize-DirectoryStructure,
    Test-DirectoryStructure, Get-DirectoryStatus, Backup-ExistingFile, New-ProjectManual
